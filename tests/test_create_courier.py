...
class TestCreateCourier:
    ...
    def test_create_courier_success_status(self):
        payload = new_courier.new_login_pass_fname()
    ...
    def test_create_courier_success_response_ok_true(self):
        payload = new_courier.new_login_pass_fname()
        url = Url.BASE_URL + Url.COURIER_URL
        response = requests.post(url=url, json=payload)
    ...
    def test_create_courier_two_similar_couriers_status_code_409(self):
        payload = new_courier.new_courier()
        url = Url.BASE_URL + Url.COURIER_URL
        response = requests.post(url=url, json=payload)
        assert response.status_code == Td.conflict_status

    @allure.title('Проверка создания одинаковых курьеров. Сообщение об ошибке.')   
    def test_create_courier_two_similar_couriers_message(self):
        payload = new_courier.new_courier()
        url = Url.BASE_URL + Url.COURIER_URL
        response = requests.post(url=url, json=payload)
